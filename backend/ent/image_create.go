...
func (ic *ImageCreate) SetIdentifier(s string) *ImageCreate {
...
func (ic *ImageCreate) check() error {
	if _, ok := ic.mutation.Identifier(); !ok {
		return &ValidationError{Name: "identifier", err: errors.New(`ent: missing required field "Image.identifier"`)}
	}
	if _, ok := ic.mutation.CreatedAt(); !ok {
